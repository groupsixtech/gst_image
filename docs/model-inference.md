...
The workflow runs the stock `cpsam_v2` model locally on CPU by default, or on a
locally available NVIDIA CUDA GPU selected in the dialog or with `--device gpu`.
It never uploads a source image. If the model is not already in Cellpose's local cache, the user
