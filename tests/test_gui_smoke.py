...
def test_analysis_sliders_and_spin_boxes_stay_synchronized(qtbot):
    window = MainWindow()
    qtbot.addWidget(window)
    window.manual_threshold_slider.setValue(211)
    assert window.manual_threshold.value() == 211
...
def test_threshold_method_shows_only_applicable_controls(qtbot):
    ...
    assert window.manual_threshold_control.isHidden()
    ...
    assert not window.manual_threshold_control.isHidden()
...
def test_restore_analysis_defaults_resets_recipe_and_resolution_controls(qtbot):
    ...
    window.manual_threshold.setValue(231)
